...
from ..core import WesternCalendar, MON, TUE, FRI, SAT, SUN
...
class NewZealand(WesternCalendar):
    ...
    def get_canterbury_anniversary(self, year):
        return (
            NewZealand.get_nth_weekday_in_month(year, 11, FRI, 2, start=NewZealand.get_nth_weekday_in_month(year, 11, TUE, 1)),
    ...
    def get_variable_days(self, year):
        ...
        days.append(self.get_canterbury_anniversary(year))
        ...
        matariki = date(year, 7, 14)
        if matariki.weekday() in self.get_weekend_days():
            days.append((
                self.find_following_working_day(matariki), "Matariki Shift")
            )
